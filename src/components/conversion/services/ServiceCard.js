  ...
  return (
    ...
    >
      <div className="flex items-center mb-3">
        ...
        <h3 className="service-title text-lg font-bold text-dark dark:text-light line-clamp-1">{tl(title, language)}</h3>
      </div>

      <p className="service-description text-gray-600 dark:text-gray-300 text-sm mb-3 flex-grow line-clamp-3">{tl(description, language)}</p>
      ...
      <div className="service-technologies flex flex-wrap gap-1 mb-3">
        ...
        {technologies.length > 3 && (
          <span
            className="technology-tag text-xs py-0.5 px-1.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-md cursor-default"
            title={technologies.slice(3).join(', ')}
          >
            +{technologies.length - 3}
          </span>
        )}
      ...
      {exampleProject && (
        <div className="service-example mb-3 text-xs">
          <span className="text-gray-600 dark:text-gray-400">{t.example} </span>
          <Link
